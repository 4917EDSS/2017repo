...
OI::OI()
{
	...
	driveShifterBtn.reset(new JoystickButton(driverController.get(), DRIVE_SHIFTER_BTN));
	driveShifterBtn->WhenPressed(new ToggleShifterCmd);

	driveBeltArmsOpenCloseBtn.reset(new JoystickButton(driverController.get(), DRIVE_BELTARMS_OPENCLOSE_BTN));
	driveBeltArmsOpenCloseBtn->WhenPressed(new ToggleArmsOpenCloseCmd);

	driveJawsOpenCloseBtn.reset(new JoystickButton(driverController.get(), DRIVE_JAWS_OPENCLOSE_BTN));
	driveJawsOpenCloseBtn->WhenPressed(new ToggleJawsOpenCloseCmd);

	driveJawsInOutBtn.reset(new JoystickButton(driverController.get(), DRIVE_JAWS_INOUT_BTN));
