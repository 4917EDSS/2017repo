...
void ToggleShifterCmd::Initialize() {
	if(drivetrainSub->getShifterState() == frc::DoubleSolenoid::Value::kForward){
		drivetrainSub->setShifter(frc::DoubleSolenoid::Value::kReverse);
	}
	else{
		drivetrainSub->setShifter(frc::DoubleSolenoid::Value::kForward);
	}
